 ...
 * signed integer range: [−2^31,  2^31 − 1]. If the numerical value is out of the range of representable values,
...
public class P8_Atoi {
    public int myAtoi(String str) {
        ...
        for (int i = 0; i < str.length(); i++) {
            ...
            if (i - firstNonWhiteSpaceCharIndex == 0) {
                if (ch == '-') {
                    isPositive = false;
                } else if (ch == '+') {
                    continue;
            ...
            } else if (i - firstNonWhiteSpaceCharIndex == 1 && (ch == '+' || ch == '-')) {
                return 0;
            } else {
                if (ch >= '0' && ch <= '9') {
                    res = res * 10 + (ch - '0');
